...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  CI_EXEC "${TEST_RUNNER_ENV}" DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}/bin/test_valdcoin" --catch_system_errors=no -l test_suite
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  set -eo pipefail
  export P_CI_DIR="${BASE_BUILD_DIR}/valdcoin-$HOST/src/"
  ...
  CI_EXEC "python3 ${DIR_IWYU}/include-what-you-use/iwyu_tool.py"\
          ...
          " src/zmq"\
          " -p . ${MAKEJOBS}"\
          " -- -Xiwyu --cxx17ns -Xiwyu --mapping_file=${BASE_BUILD_DIR}/valdcoin-$HOST/contrib/devtools/iwyu/valdcoin.core.imp"\
          " |& tee /tmp/iwyu_ci.out"
