#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Valdcoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} VALDCOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Valdcoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Valdcoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example valdcoin.conf file. if contrib/devtools/gen-valdcoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/valdcoin.conf" "${DISTNAME}/"
