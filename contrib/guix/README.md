# Bootstrappable Valdcoin Core Builds

This directory contains the files necessary to perform bootstrappable Valdcoin
Core builds.
...
To build Valdcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Valdcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/valdcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/valdcoin-core/guix.sigs).
