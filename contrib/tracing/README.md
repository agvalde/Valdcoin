...
Tracing (USDT) support for Valdcoin Core on Linux using. For more information on
USDT support in Valdcoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `valdcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `valdcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Valdcoin Core USDT documentation] on how to list available tracepoints in your
`valdcoind` binary.

[Valdcoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/valdcoind
```
...
Valdcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/valdcoind
```
...
In a different terminal, starting Valdcoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/valdcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/valdcoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/valdcoind
```
