...
""" Example logging Valdcoin Core mempool events using the mempool:added,
...
def main(valdcoind_path):
    valdcoind_with_usdts = USDT(path=str(valdcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    valdcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    valdcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    valdcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    valdcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[valdcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/valdcoind")
        exit(1)
