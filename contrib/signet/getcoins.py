#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Valdcoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing valdcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='valdcoin-cli', help='valdcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Valdcoin address to which the faucet should send')
...
parser.add_argument('valdcoin_cli_args', nargs='*', help='Arguments to pass on to valdcoin-cli (default: -signet)')

args = parser.parse_args()

if args.valdcoin_cli_args == []:
    args.valdcoin_cli_args = ['-signet']


def valdcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.valdcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = valdcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = valdcoin_cli(['getnewaddress', 'faucet', 'bech32'])
