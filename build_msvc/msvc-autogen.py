...
def parse_config_into_vld_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Valdcoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/valdcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/valdcoin_config.h'), "w", encoding="utf8") as vld_config:
        vld_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Valdcoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_vld_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/valdcoin_config.h'), os.path.join(SOURCE_DIR, 'config/valdcoin-config.h'))
