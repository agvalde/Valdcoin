...
`test/functional/test_framework/test_framework.py:ValdcoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_valdcoin_cli.py --legacy-wallet',
    'interface_valdcoin_cli.py --descriptors',
...
def main():
    ...
    enable_valdcoind = config["components"].getboolean("ENABLE_VALDCOIND")

    if not enable_valdcoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if valdcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "valdcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a valdcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `valdcoin-cli help` (`rpc_interface.txt`).
