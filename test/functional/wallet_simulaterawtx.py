#!/usr/bin/env python3
# Copyright (c) 2021-2022 The Valdcoin Core developers
...
class SimulateTxTest(ValdcoinTestFramework):
    ...
    def run_test(self):
        ...
        valdcoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 vld decrease, w2 sees + 5 vld
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + valdcoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - valdcoin_fee + 4.9999       -5 - valdcoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - valdcoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - valdcoin_fee)
        ...
        tx2 = funding["hex"]
        valdcoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + valdcoin_fee2))
