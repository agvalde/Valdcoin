...
random value payments which sum to 10VLD for each output type to Bob,
for a total of 40VLD in Bob's wallet.
...
class AddressInputTypeGrouping(ValdcoinTestFramework):
    ...
    def make_payment(self, A, B, v, addr_type):
        ...
        self.log.debug(f"Making payment of {v} VLD at fee_rate {fee_rate}")
        tx = B.sendtoaddress(
    ...
    def run_test(self):
        ...
        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} VLD to legacy")
            A.sendtoaddress(B.getnewaddress(address_type="legacy"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} VLD to p2sh")
            A.sendtoaddress(B.getnewaddress(address_type="p2sh-segwit"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} VLD to bech32")
            A.sendtoaddress(B.getnewaddress(address_type="bech32"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} VLD to bech32m")
