...
def assert_fee_amount(fee, tx_size, feerate_VLD_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_VLD_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s VLD too low! (Should be %s VLD)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_VLD_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s VLD too high! (Should be %s VLD)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting VLD values"""
...
def get_fee(tx_size, feerate_vld_kvb):
    """Calculate the fee in VLD given a feerate is VLD/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_vld_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  VLD
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `ValdcoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "valdcoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "valdcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "valdcoin.conf")):
        with open(os.path.join(datadir, "valdcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
