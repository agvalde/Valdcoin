#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Valdcoin Core developers
...
TMPDIR_PREFIX = "valdcoin_func_test_"
...
class ValdcoinTestMetaClass(type):
    """Metaclass for ValdcoinTestFramework.

    Ensures that any attempt to register a subclass of `ValdcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ValdcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ValdcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ValdcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ValdcoinTestFramework(metaclass=ValdcoinTestMetaClass):
    """Base class for a valdcoin test script.

    Individual valdcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave valdcoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop valdcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use valdcoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_valdcoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "valdcoind" + config["environment"]["EXEEXT"],
        )
        fname_valdcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "valdcoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_valdcoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "valdcoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.valdcoind = os.getenv("VALDCOIND", default=fname_valdcoind)
        self.options.valdcoincli = os.getenv("VALDCOINCLI", default=fname_valdcoincli)
        self.options.valdcoinutil = os.getenv("VALDCOINUTIL", default=fname_valdcoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: valdcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ValdcoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'valdcoind', self.options.valdcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'valdcoin-cli', self.options.valdcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a valdcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple valdcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as valdcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    valdcoind=self.options.valdcoind,
                    valdcoin_cli=self.options.valdcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in valdcoin.conf
    ...
    def skip_if_no_valdcoind_tracepoints(self):
        """Skip the running test if valdcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("valdcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_valdcoind_zmq(self):
        """Skip the running test if valdcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("valdcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if valdcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("valdcoin-wallet has not been compiled")

    def skip_if_no_valdcoin_util(self):
        """Skip the running test if valdcoin-util has not been compiled."""
        if not self.is_valdcoin_util_compiled():
            raise SkipTest("valdcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if valdcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("valdcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether valdcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether valdcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_valdcoin_util_compiled(self):
        """Checks whether valdcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_VALDCOIN_UTIL")
