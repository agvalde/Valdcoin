...
"""Valdcoin test framework primitive and message structures
...
COIN = 100000000  # 1 vld in satoshis
...
# Objects that map to valdcoind objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/valdcoin/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # Valdcoin Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in valdcoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in valdcoind
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in valdcoind indicates these should be deserialized as blocks
