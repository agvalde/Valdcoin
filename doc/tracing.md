# User-space, Statically Defined Tracing (USDT) for Valdcoin Core

Valdcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Valdcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Valdcoin Core
...
Multiple tools can list the available tracepoints in a `valdcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Valdcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/valdcoind
stap net        outbound_message 0x0000000000107c05 /src/valdcoind
stap validation block_connected  0x00000000002fb10c /src/valdcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Valdcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/valdcoind | grep NT_STAPSDT -A 4 -B 2
