...
The Valdcoin Core daemon can be configured to act as a trusted "border
router", implementing the valdcoin wire protocol and relay, making
...
The ZeroMQ feature in Valdcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building valdcoind:
...
For instance:
    ...
    $ valdcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/valdcoind.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in valdcoin.conf.
...
From the perspective of valdcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into valdcoind directly. Furthermore, no information is
...
using. Valdcoind appends an up-counting sequence number to each
