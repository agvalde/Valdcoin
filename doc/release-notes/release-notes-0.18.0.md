Valdcoin Core version 0.18.0 is now available from:

  <https://valdcoincore.org/bin/valdcoin-core-0.18.0/>
...
`/Applications/Valdcoin-Qt` (on Mac) or `valdcoind`/`valdcoin-qt` (on
Linux).
...
recommended to use Valdcoin Core on unsupported systems.

Valdcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Valdcoin Core does not yet change appearance when
...
`contrib/init/valdcoind.service`, it has been changed to use
`/var/lib/valdcoind` as the data directory instead of
`~valdcoin/.valdcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/valdcoind` will
exist has enough space (check using `df -h /var/lib/valdcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/valdcoin/valdcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://valdcoincore.org/en/contact/) the Valdcoin Core
...
The systemd init file (`contrib/init/valdcoind.service`) has been changed
to use `/var/lib/valdcoind` as the data directory instead of
`~valdcoin/.valdcoin`. This change makes Valdcoin Core more consistent with
...
`datadir` option in `/etc/valdcoin/valdcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/valdcoin/valdcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/valdcoin/valdcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/valdcoin/valdcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/valdcoin/valdcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/valdcoin/valdcoin/blob/master/doc/valdcoin-conf.md)
  about the `valdcoin.conf` file describes how to use it to configure
  Valdcoin Core.

- A new document introduces Valdcoin Core's BIP174 [Partially-Signed
  Valdcoin Transactions
  (PSBT)](https://github.com/valdcoin/valdcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/valdcoin/valdcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Valdcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Valdcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/valdcoin/valdcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/valdcoin/valdcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Valdcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Valdcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/valdcoin/valdcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/valdcoin/valdcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart valdcoind with the `-deprecatedrpc=generate`
...
- A new `valdcoin-wallet` tool is now distributed alongside Valdcoin
...
This section describes planned changes to Valdcoin Core that may affect
other Valdcoin software and services.

- Since version 0.16.0, Valdcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Valdcoin Core 0.20 (expected about
  a year after 0.18), Valdcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Valdcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Valdcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Valdcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Valdcoin Core
...
- A sub-project of Valdcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Valdcoin Core.  See their [project
  page](https://github.com/valdcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Valdcoin Core's own implementation, although entropy
  gathered by Valdcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Valdcoin Core a
...
- On macOS, Valdcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `valdcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libvaldcoin_server.a` (jnewbery)
...
- #13926 [Tools] valdcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like valdcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `valdcoin-qt -version` output formatting (hebasto)
...
- #11625 Add ValdcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libvaldcoin_server` (laanwj)
...
- #15154 Configure: valdcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/valdcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Valdcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libvaldcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in valdcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Valdcoin Core (Christewart)
...
- #12246 Bugfix: Only run valdcoin-tx tests when valdcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_valdcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_valdcoin` (MarcoFalke)
...
- #14186 valdcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_valdcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update valdcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/valdcoin-conf.md (hebasto)
...
- #15354 Add missing `valdcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct valdcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support valdcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/valdcoin/).
