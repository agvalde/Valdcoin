Valdcoin Core version 0.11.0 is now available from:

  <https://valdcoin.org/bin/valdcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer VLD/kB fee to be
...
For example, add the following to `valdcoin.conf`:
...
related to the blockchain in the valdcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Valdcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Valdcoin Core will 
...
For an example script see [valdcoin-submittx](https://github.com/laanwj/valdcoin-submittx).
...
- #5334 `cf87536` libvaldcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new valdcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ValdcoinAmountField
- #5753 `0f44672` Add valdcoin logo to about screen
...
- #5809 `46bfbe7` Add valdcoin-cli man page
...
- #5849 `41259ca` contrib/init/valdcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Valdcoin-Qt.app to Valdcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for valdcoin-tx)
