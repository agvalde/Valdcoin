# CJDNS support in Valdcoin Core

It is possible to run Valdcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Valdcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Valdcoin network and individual nodes.
...
before it will work with your Valdcoin Core node. This is described in steps
...
## Run Valdcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Valdcoin Core
...
When enabled, this option tells Valdcoin Core that it is running in an
...
IPv6 local network. This helps Valdcoin Core perform better address management:
...
CJDNS support was added to Valdcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `valdcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/valdcoin/valdcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Valdcoin Core

There are several ways to see your CJDNS address in Valdcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `valdcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `valdcoin-cli getpeerinfo`).
