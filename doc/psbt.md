# PSBT Howto for Valdcoin Core

Since Valdcoin Core 0.17, an RPC interface exists for Partially Signed Valdcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/valdcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Valdcoin transactions that are not fully signed
...
#### Multisig with multiple Valdcoin Core instances
...
Valdcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `valdcoin-cli` is used.
...
Later, when *V* VLD has been received on *Amulti*, and Bob and Carol want to
