...
Valdcoin Core version 25.0 is now available from:

  <https://valdcoincore.org/bin/valdcoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `valdcoin-cli` tool both take advantage of this, so
  for example:

```sh
valdcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
valdcoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Valdcoin Core will now return an error at startup. (#25574)
...
before Valdcoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching valdcoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/valdcoin/valdcoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
