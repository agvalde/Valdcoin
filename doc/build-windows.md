...
Below are some notes on how to build Valdcoin Core for Windows.

The options known to work for building Valdcoin Core on Windows are:
...
Note that for WSL the Valdcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/valdcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\valdcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/valdcoin
