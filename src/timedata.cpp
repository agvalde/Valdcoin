...
#define VALDCOIN_TIMEDATA_MAX_SAMPLES 200

static std::set<CNetAddr> g_sources;
static CMedianFilter<int64_t> g_time_offsets{VALDCOIN_TIMEDATA_MAX_SAMPLES, 0};
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    LOCK(g_timeoffset_mutex);
    // Ignore duplicates
    if (g_sources.size() == VALDCOIN_TIMEDATA_MAX_SAMPLES)
        return;
...
void TestOnlyResetTimeData()
{
    ...
    nTimeOffset = 0;
    g_sources.clear();
    g_time_offsets = CMedianFilter<int64_t>{VALDCOIN_TIMEDATA_MAX_SAMPLES, 0};
