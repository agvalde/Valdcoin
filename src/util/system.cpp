...
const char * const VALDCOIN_CONF_FILENAME = "valdcoin.conf";
const char * const VALDCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //valdcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", VALDCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Valdcoin
    // macOS: ~/Library/Application Support/Valdcoin
    // Unix-like: ~/.valdcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Valdcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Valdcoin";
#else
    // Unix-like
    return pathRet / ".valdcoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", VALDCOIN_CONF_FILENAME));
}
