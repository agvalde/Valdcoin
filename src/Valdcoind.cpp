...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/valdcoin.conf are parsed in qt/valdcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  valdcoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see valdcoind -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for valdcoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect valdcoind signal handlers
    noui_connect();
