...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ValdcoinUnit>("ValdcoinUnit");
#else
    qRegisterMetaType<ValdcoinUnit>("ValdcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. valdcoin_de.qm (shortcut "de" needs to be defined in valdcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. valdcoin_de_DE.qm (shortcut "de_DE" needs to be defined in valdcoin.qrc)
...
static const char* qt_argv = "valdcoin-qt";

ValdcoinApplication::ValdcoinApplication()
...
void ValdcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ValdcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ValdcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ValdcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ValdcoinApplication::createOptionsModel(bool resetSettings)
{
...
void ValdcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ValdcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ValdcoinGUI::quitRequested, this, &ValdcoinApplication::requestShutdown);
...
void ValdcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ValdcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ValdcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ValdcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ValdcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ValdcoinApplication::handleRunawayException);
    connect(this, &ValdcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ValdcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ValdcoinApplication::parameterSetup()
{
...
void ValdcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ValdcoinApplication::requestInitialize()
{
...
void ValdcoinApplication::requestShutdown()
{
...
void ValdcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // valdcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ValdcoinGUI::handlePaymentRequest);
            connect(window, &ValdcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ValdcoinApplication::handleRunawayException(const QString &message)
{
...
void ValdcoinApplication::handleNonFatalException(const QString& message)
{
...
WId ValdcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ValdcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ValdcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(valdcoin);
    Q_INIT_RESOURCE(valdcoin_locale);
    ...
    /// 6-7. Parse valdcoin.conf, determine network, switch to network specific
    ...
    // valdcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
