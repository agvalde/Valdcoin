...
#include <qt/valdcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Valdcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ValdcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ValdcoinAddressCheckValidator(parent));
}
...
bool parseValdcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no valdcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("valdcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ValdcoinUnits::parse(ValdcoinUnit::VLD, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseValdcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseValdcoinURI(uriInstance, out);
}

QString formatValdcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("valdcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ValdcoinUnits::format(ValdcoinUnit::VLD, info.amount, false, ValdcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openValdcoinConf()
{
    ...
    /* Open valdcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Valdcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Valdcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Valdcoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Valdcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "valdcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("valdcoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a valdcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Valdcoin\n";
        else
            optionFile << strprintf("Name=Valdcoin (%s)\n", chain);
