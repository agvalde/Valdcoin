...
/** Call node.updateRwSetting() with Valdcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Valdcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/valdcoin/valdcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because valdcoin 22.x will not try to read these.
...
//! Convert enabled/size values to valdcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from valdcoin -prune setting.
...
//! Get pruning size value to show in GUI from valdcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayValdcoinUnit")) {
        settings.setValue("DisplayValdcoinUnit", QVariant::fromValue(ValdcoinUnit::VLD));
    }
    QVariant unit = settings.value("DisplayValdcoinUnit");
    if (unit.canConvert<ValdcoinUnit>()) {
        m_display_valdcoin_unit = unit.value<ValdcoinUnit>();
    } else {
        m_display_valdcoin_unit = ValdcoinUnit::VLD;
        settings.setValue("DisplayValdcoinUnit", QVariant::fromValue(m_display_valdcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if valdcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_valdcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ValdcoinUnit>() == m_display_valdcoin_unit) return;
    m_display_valdcoin_unit = new_unit.value<ValdcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayValdcoinUnit", QVariant::fromValue(m_display_valdcoin_unit));
    Q_EMIT displayUnitChanged(m_display_valdcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/valdcoin/valdcoin/pull/8273
    ...
    // (https://github.com/valdcoin-core/gui/issues/567).
