...
#include <config/valdcoin-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatValdcoinURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(ValdcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatValdcoinURI(info));
}
